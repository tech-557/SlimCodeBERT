...
def delete_code_pattern(code):
    ...
    global pruned_length
    origin_length += len(code)
    reduction = Code_Reduction(code)
    ...
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        with open('/'.join(index_filename.split('/')[:-1])+'/data.jsonl') as f:
            for line in f:
                line=line.strip()
                js=json.loads(line)
                url_to_code[js['idx']]=delete_code_pattern(js['func'])
...
def train(args, train_dataset, model, tokenizer,pool):
    ...
    with open('./rate', 'w') as f:
        f.write(str(origin_length) + '\n')
        f.write(str(pruned_length) + '\n')
        f.write(str(pruned_length / origin_length))
