...
from token_frequence import read_token_freq, prune_token
import re
import os
import sys
import bleu
...
def read_examples(filename, strategy='origin'):
    ...
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            if strategy == 'identifiers':
                import javalang
                tokens = javalang.tokenizer.tokenize('public class Clazz {' + code + '}')
                lst = []
                for token in tokens:
...
def generate_low_rated_tokens(token_file_dir='./', pruned_rate=0.3):
    low_rated_tokens = []
    with open(token_file_dir, 'r') as f:
        ...
        while True:
            ...
            if not token:
                break
            low_rated_tokens.append(token)
    return low_rated_tokens


def prune_tokens(code, low_rated_tokens):
    ...
    def is_not_low_rated_token(token):
        return token not in low_rated_tokens
    tokens = code.split(' ')
    result = []
    for token in tokens:
        camel_case_words = camel_case_split(token)
        low_rated_words = map(is_not_low_rated_token, camel_case_words)
        result.append(low_rated_words)
    return result
...
def delete_token_pattern(tokens, strategy='None', lang='java'):
    result = []
    if strategy == 'frequence_token':
        token_map = read_token_freq()
        result = prune_token(tokens, token_map)
    return result


def delete_code_with_variable_declaration(code):
    start, end = 0, 0
    statements = []
...
def convert_examples_to_features(examples, tokenizer, args, stage=None, lang='python'):
    features = []
    origin_code_length = 0
    pruned_code_length = 0
    for example_index, example in enumerate(examples):
        # origin_code_length += len(example.source)
        example.source = delete_code_pattern(example.source, strategy=args.prune_strategy, lang=args.lang)
        # pruned_code_length += len(example.source)

        # source
        source_tokens = tokenizer.tokenize(example.source)
        source_tokens = delete_token_pattern(source_tokens, strategy=args.prune_strategy, lang=args.lang)
    ...
    with open('./reduction_'+str(args.prune_strategy)+'_'+str(args.lang), 'w') as f:
...
def main():
    ...
    parser.add_argument('--prune_strategy', default='None', type=str, help='pruning strategy slim, random, identifiers, or frequent_token')
    # print arguments
    ...
    if args.do_train:
        ...
        train_examples = read_examples(args.train_filename, strategy=args.prune_strategy)
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) % args.eval_steps == 0) and eval_flag:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = read_examples(args.dev_filename, strategy=args.prune_strategy)
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            eval_examples = read_examples(file, strategy=args.prune_strategy)
            eval_features = convert_examples_to_features(eval_examples, tokenizer, args, stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)
            eval_data = TensorDataset(all_source_ids, all_source_mask)
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    # flops
                    logger.info("flops------")
                    inputs = {'source_ids': source_ids, 'source_mask': source_mask}
                    macs, params = profile(model, inputs=inputs, verbose=False)
                    from thop import clever_format
                    macs, params = clever_format([macs, params], "%.3f")
                    logger.info(macs)
                    logger.info(params)
                    logger.info("end flops------")
                    sys.exit()
